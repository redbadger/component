...
request = request.defaults({ 'proxy': process.env.HTTPS_PROXY || process.env.https_proxy });
/**
 * In-flight requests.
 */
...
function Package(pkg, version, options) {
  ...
  if (inFlight[this.slug]) this.install = this.emit.bind(this, 'end');
...
Package.prototype.getJSON = function(fn){
  ...
  request.get(url, function(err, res) {
    if (err) return fn(error(err, url));
    try {
      debug('got %s', url);
  ...
  files.forEach(function(file){
    batch.push(function(done){
      ...
      self.mkdir(dirname(dst), function(err){
        ...
        var req = request.get(url).pipe(fs.createWriteStream(dst))
        req.on('error', done);
        req.on('end', done);
      });
    });
...
function error(error, url) {
  var err = new Error('failed to fetch ' + url + ', got ' + error);
  return error;
}
