...
if (ca_dir) {
  fs.readdir(ca_dir, function(err, files) {
    if (err) throw err

    options.ca = files.map(function (file) {
      var absolutePath = path.resolve(ca_dir, file);
...
exports.register = function(username, project, conf, fn){
  ...
  request.post({ 'uri': url, 'body': conf }, function(err, res) {
    if (err) return fn(err);
